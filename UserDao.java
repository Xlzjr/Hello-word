...
public Collection query(String strif) {
	...
	try {
		while(rs.next())
		{
			...
			userbean.setUid(Integer.valueOf(rs.getString(1)));
			...
			userbean.setAdress(rs.getString(7));
...
//用于修改的查询
	public UserBean queryM(UserBean userbean) {
		...
		if(userbean.getUid()!=null) {
			sql="select * from b_user where id= "+userbean.getUid()+"";
		}
		...
		try {
			while(rs.next())
			{
				...
				userbean1.setUid(Integer.valueOf(rs.getString(1)));
				userbean1.setName(rs.getString(2));
				userbean1.setPwd(rs.getString(3));
				userbean1.setSex(rs.getString(4));
				userbean1.setEmail(rs.getString(5));
				userbean1.setPhone(rs.getString(6));
				userbean1.setAdress(rs.getString(7));
	...
	public int insert(UserBean userbean) {
		String sql1="select * from b_user where username='"+userbean.getName()+"'";
	...
	// 修改数据
		public int update(UserBean userbean) {
			int flag=0;
			String sql = "";
			sql = "Update b_user set username = '" + userbean.getName()
			+"', pwd = '" + userbean.getPwd()
			+"', sex = '" + userbean.getSex()
			+"', phone= '" + userbean.getPhone()
			+"', adress= '" + userbean.getAdress()
			+"' where id = " + userbean.getUid();
			flag = conn.executeUpdate(sql);
		...
		public int delete(UserBean userbean) {
			String sql = "Delete from b_user where id=" + userbean.getUid()
					+ "";
